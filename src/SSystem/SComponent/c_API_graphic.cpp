...
void cAPIGph_Painter() {
    ...
    if (dusk::stereo::active()) {
        // Update the close-up and FoV separation scales once per simulation
        // frame, BEFORE the eye loop, so both eyes use the same smoothed
        // scales.
        dusk::stereo::closeup_scale_tick();
        dusk::stereo::fov_scale_tick();
